...
public class ResponseHeaderPrintServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        resp.setContentType("text/html");           // response 객체를 생성할 때는 MIME 타입 및 인코딩을 고려
        ...
        out.flush();            // close 내부에 flush가 들어있지만 의도적으로 끝낼 시점에 flush 작성
        out.close();

        /* 설명. response 객체의 header값에 대한 키값과 밸류 값을 한번에 확인해 보기. */
        Collection<String> responseHeaders = resp.getHeaderNames();
        Iterator<String> iter = responseHeaders.iterator();     // Collection을 Iterator로 변환
        while (iter.hasNext()) {
